...
<ul class="subsubsub">
	...
	protected function enqueue_report( $is_insert = false ) {
		...
		if ( ! $is_insert ) {
			wp_register_script( 'mobile-detect', $src_mobile_detect, array(), '1.4.5', true );
			wp_enqueue_script(
	...
	protected function enqueue_view( $is_insert = false ) {
		...
		if ( $is_insert ) {
			...
			wp_register_script( 'chroma', $src_chroma, array(), '2.4.2', true );
			wp_register_script( 'aurora-heatmap-drawer', $src_h337, array(), '2.0.5', true );
			wp_enqueue_script(
				'aurora-heatmap-viewer',
				$src_viewer,
				array( 'jquery', 'chroma', 'aurora-heatmap-drawer' ),
				$this::VERSION,
				false
			);
